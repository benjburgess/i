...
This tutorial is meant to be a relatively simple example of SVMs. For an example of SVMs with more complex data, see the [Advanced Tutorial on SVMs](https://benjburgess.github.io/i/SVM_radial).
...
The next decision we need to make is what values for the cost parameter we should specify. For a refresher on this parameter, see the [initial explanation of support vector machines (SVMs)](https://benjburgess.github.io/svm). Fortunately, the SVM package we are using (`e1071`) allows us to test various different parameter values and then select the best ones.
...
The range of parameter values to be used is usually at the discretion of the user, although it is usually on a logarithmic scale. Hsu et al. (2003) suggest that values for cost could be $2^{-5}$, $2^{-3}$, ... $2^{15}$. However, for this analysis we will be using cost parameter values of $2^{-4}$, $2^{-3.5}$, $2^{-3}$, ... $2^{6}$.

When training the SVM, the best model parameters are chosen by those which generate an SVM which minimises species being assigned to the incorrect class (i.e., the model which correctly assigns most data points to the correct class). As part of this procedure, we will be using k-fold cross validation as way of increasing the robustness of our results. I'm not going to go into k-fold cross validation here, but for more information see [this explanation](https://machinelearningmastery.com/k-fold-cross-validation/).

Following this, our SVM can be trained using the following code.


```{r class.source="bg-info", class.output="bg-success"}
svm_trained  <- tune.svm(species ~ ., #determine species from weight and length data
                       data=df.train, #using the df.train dataset
                       tunecontrol = tune.control(cross=10), # use k-fold cross validation with k=10
                       cost=2^c(seq(-4, 6, by=0.5))) #select our range of cost values
...
So, as we can see from the model training, the best performing model was when cost equals `r as.numeric(svm_trained$best.parameters[1])`. Here the best performance of the model was `r as.numeric(svm_trained$best.performance)` which means that across all 10 folds of the cross validation for these parameter values, ~`r 100*(1-as.numeric(svm_trained$best.performance))`% of our data points were assigned the correct species. This is really good, but there is the potential to further increase this by tuning our SVM. 
...
At this stage we have trained the svm and identified the initial parameter value for cost. In keeping with the recommendations of [Hsu et al. (2003)](https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf) the next stage of the model development is to tune the model. At this stage, we will use a much more narrow range of parameter values for cost which should allow us to identify a better parameter value for our SVM. At this stage for the cost parameters, we now consider values in the following range which is centerd on the best model parameter from the training stage of the model development: $2^{n - 0.5}$, $2^{n - 0.375}$, $2^{n - 0.25}$, ... $2^{n+0.5}$. 
...
```{r class.source="bg-info", class.output="bg-success"}
svm_tuned  <- tune.svm(species ~ ., #determine species from weight and length data
                       data=df.train, #using the df.train dataset again
                       tunecontrol = tune.control(cross=10),  # use k-fold cross validation with k=10
                        cost=2^c(seq(log(svm_trained_model$cost, base=2)-0.5, 
                                     log(svm_trained_model$cost, base=2)+0.5, by=1/8))) #select our range of cost values

summary(svm_tuned)

```

So, as we can see from our tuning of the SVM, the best performing model has now changed to where cost equals `r as.numeric(svm_tuned$best.parameters[1])`. Here the best performance of the model has improved to `r as.numeric(svm_trained$best.performance)` which means that across all 10 folds of the cross validation for these parameter values, ~`r 100*(1-as.numeric(svm_trained$best.performance))`% of our data points were assigned the correct species. At this stage we'll move from tuning our SVM through to the testing stage.
...
So as we discussed above, the best forming model is where cost equals `r as.numeric(svm_tuned$best.parameters[1])`. Here across all 10 folds of the cross validation ~`r 100*(1-as.numeric(svm_tuned$best.performance))`% of our data points were assigned the correct species. This can be seen using the following code.
...
Overall, our SVM does a great job of correctly classifying the two species in our dataset. In this there is a relatively defined boundary between the two species, hence the SVM is able to very accurately classify our data. If there was not such a clearly defined boundary, then we might want to consider using a different kernel (e.g., radial or polynomial as opposed to linear). Similarly, if we had more than two species we may find that our SVM doesn't do such a good job of classifying our data.
