...
final_svm_model  <- svm_trained$best.model
...
So as we discussed above, the best forming model is where cost equals `r as.numeric(svm_trained$best.parameters[1])`. Here across all 10 folds of the cross validation ~`r 100*(1-as.numeric(svm_trained$best.performance))`% of our data points were assigned the correct species. This can be seen using the following code.
...
If we start with accuracy, the accuracy of our SVM for the test dataset is `r as.numeric(cm$overall[1])*100`% which is pretty similar to the accuracy we obtained from the k-fold cross validation of the SVM at the training stage (~`r 100*(1-as.numeric(svm_trained$best.performance))`%). In other words, our SVM classifies the vast majority of our data points correctly. As such, our model does a really good job of classifying different species in our data. 
