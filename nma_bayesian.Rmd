---
title: "Bayesian Network Meta-analyses in R"
output:
  html_document:
    fig_caption: yes
    theme: cerulean
...
Network meta-analyses (NMA) are a powerful statistical tool frequently employed in medical research. As briefly described in the overview of NMAs, there are two different types, namely frequentist or Bayesian NMAs. Here, I'm going to focus on Bayesian NMAs, I won't go into the theory surronding Bayes theroem, but there are plenty of great resources online. Likewise, I'm going to assume that you have a basic understanding of meta-analyses, although if not have a look at this [extensive resource](https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/).

There are a variety of different programs which can be used to conduct NMAs.Indeed, the recently published [NICE Meta-Analysis Guideline Methodology Document](https://www.bristol.ac.uk/media-library/sites/social-community-medicine/documents/mpes/gmd-1-meta-analysis-jan2021.pdf) outlines the various different options that could be used for NMAs (as well as those which should be avoided).

Here, I've written a short introductory guide to using the *gemtc* package in *R* to conduct Bayesian-NMAs. Rather than use a well-known or common dataset to showcase this approach I've decided to replicate an previously published analysis. 

The recently published study by [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863) uses a Bayesian-NMA to compare the effect of various sodium-glucose co-transporter inhibitors and metformin in patients with type 1 diabetes. In doing so they collate data from nine studies for the change from baseline (CFB) of HbA1c, weight, total daily insulin dose, and systolic blood pressure at two time points (24-26 weeks and 52 weeks). To analyse their data using a Bayesian-NMA the authors used the *WINBUGS* software. Accordingly, here we will replicate their analysis in *R* using the *gemtc* package and compare our results to those from the original paper. Rather than focus on all four of the metrics, we are just going to focus on the effect of the various treatments on HbA1c, although the same approach could be used for all other response metrics.
...
As I mentioned above, the data we will be using here is for the CFB in HbA1c under each treatment. For these studies, the aim was for any treatment to reduce the prevalence of HbA1c. Hence, if a treatment has a beneficial effect on HbA1c, the effect size should be negative. Here, CFB in HbA1c is measured in percentage (%) and the effect sizes we have calculated are mean differences. In the original study, Langford et al. compare each treatment to either a placebo or to the standard treatment of Metformin (2000mg), accordingly in this tutorial we will replicate both of these analyses.
...
The next step is to load in the data for this analysis. [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863) make this data openly available in the supplementary information of their paper which I have put into a csvs file for this analysis. As a quick check of their data, I also extracted the data from their studies myself and found that it aligned well with the data provided by [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863).
...
At this point we will also load data on the cumulative number of patients for each of the treatments at each time point. We're not going to need this for our analysis, but will be using it to help visualise our data in the network plot below.
...
#format the treatment names for clarity
...
For our models, it appears as though the Bayesian-NMA which incorporates random effects is the best fitting model, hence we will use the random effects model for the rest of the analysis of the 24-26 week HbA1c data. This results matches the findings of Langford et al. who also report that their random effects model had a lower DIC compared to their fixed effects model.

Additionally the $I^{2}$ metric for the random effects model is 0% which indicates consistency in effect sizes across the different studies.
...
Another attribute of our NMA which we can assess is the Potential Scale Reduction Factor (PSRF) which compares any varitation both within and between our chains. Ideally, this value should tend to zero with an increasing number of iterations but at the very least should be close to 1 and no more 1.05. We can assess this using Gelman-Rubin plots. Although we can generate these plots for each comparison (e.g., Metformin 2000mg v Placebo, Dapagliflozin 5mg v Placebo etc.) we will again only show the comparison of Metformin 2000mg v Placebo for clarity.
...
An additional way to rank the various treatments is to use a metric called Surface Under the Cumulative Ranking (SUCRA). As before, this produces a histogram which can be used to rank the various treatments - although this was not previously reported by Langford et al.. The code to generate a plot of SUCRA values for each treatment is as follows.
...
In their paper, Langford et al. report forest plots showcasing how the various treatments compare either to the placebo treatment or the Metformin (2000mg) treatment. As such, we can likewise generate these forest for our model and compare our results to those reported by Langford et al..
...
This forest plot shows the estimated effect sizes for the mean difference (%) in CFB HbA1c for each treatment relative to Metformin (2000mg). Here, each effect size estimate has been determined using our random effects Bayesian-NMA, with the error bars representing the 95% credible intervals for ech effect size. Our analysis suggests that most treatments, at 24-26 weeks, led to significant reductions in HbA1c compared to the Metformin (2000mg) treatment. However, one exception to this was that the Metformin (2000mg) treatment was significantly better at reducing HbA1c compared to the placebo treatment. Likewise, given that the 95% credible interval for the Empagliflozin (2.5mg) treatment overlap zero, there is no statistically significant difference between the Empagliflozin (2.5mg) and Metformin (2000mg) treatments. Additionally, magnitude of any effect substantially varied between treatments, although there is substantial overlap between the credible intervals of each treatment. 

The figure we have generated here can be directly compared to Figure 4a from [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863). By comparing the figures, we can see the results of our analysis exactly align with those reported by Langford et al. (2020). 

Importantly, we can interpret these results in line with the network plot we generated earlier. As we can see, with the exception of the placebo treatment, there are no direct comparisons between Metformin (2000mg) and any other treatment. Accordingly, as noted by Langford et al., this means that all of the reported effect sizes are generated from indirect comparisons and as such are potentially liable to bias. This is an important when interpretting these results. 
...
We can run the Bayesian-NMAs using the following code, which again uses the same burnin period and number of iterations as that specfied by Langford et al. (2020).
...
As with the 24-26 week data, the best fitting NMA was the random effects model which had a lower DIC than the fixed effects NMA. Again, this mirrors the findings of Langford et al. (2020).
...
The results of this figure closely align with the conclusions of the rankogram, with the same treatments being identified as being either the most (Empagliflozin 25mg, 10mg) or least (Placebo, Metformin 2000mg) effective.
...
As before, the results shown in this figure exactly align to those shown in Figure 3a from Langford et al. (2020).
...
As before, the results shown in this figure exactly align to those shown in Figure 4a from Langford et al. (2020).
