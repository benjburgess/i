...
As I mentioned above, the data we will be using here is for the CFB in HbA1c under each SGLT treatment. For these studies, the aim was for any SGLT treatment to reduce the level of HbA1c. Hence, if a SGLT treatment has a beneficial effect on HbA1c, the effect size should be negative. Here, CFB in HbA1c is measured in percentage (%) and the effect sizes we have calculated are mean differences. In the original study, Langford et al. compare each SGLT treatment to either a placebo or to the standard treatment of Metformin (2000mg), accordingly in this tutorial we will replicate both of these analyses.
...
At this point we will also load data on the cumulative number of patients for each of the SGLT treatments at each time point. We're not going to need this for our analysis, but will be using it to help visualise our data in the network plot below.
...
In their paper, Langford et al. report forest plots showcasing how the various SGLT treatments compare either to the placebo treatment or the Metformin (2000mg) treatment. As such, we can likewise generate these forest for our model and compare our results to those reported by Langford et al. (2020).

####Comparison of treatments to placebo

We'll start by generating the forest plot showing the effect sizes (mean difference) in CFB HbA1c (%) for each SGLT treatment and Metformin in comparison to the placebo treatment. The code to generate this figure is a little complex, although it provides a clear figure using *ggplot* in contrast to the less clear figure generated by the *gemtc* package.
...
As before, we can also generate a forest plot showing the effect sizes (mean difference) in CFB HbA1c (%) for each SGLT treatment and the placebo in comparison to the Metformin (2000mg) treatment. This can be done using the following code.
