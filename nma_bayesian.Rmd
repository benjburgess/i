...
As I mentioned above, the data we will be using here is for the CFB in HbA1c under each treatment. For these studies, the aim was for any treatment to reduce the level of HbA1c. Hence, if a treatment has a beneficial effect on HbA1c, the effect size should be negative. Here, CFB in HbA1c is measured in percentage (%) and the effect sizes we have calculated are mean differences. In the original study, Langford et al. compare each treatment to either a placebo or to the standard treatment of Metformin (2000mg), accordingly in this tutorial we will replicate both of these analyses.
...
In their paper, Langford et al. report forest plots showcasing how the various treatments compare either to the placebo treatment or the Metformin (2000mg) treatment. As such, we can likewise generate these forest for our model and compare our results to those reported by Langford et al. (2020).
...
In their paper, Langford et al. report forest plots showcasing how the various treatments compare either to the placebo treatment or the Metformin (2000mg) treatment at 52 weeks. As such, we can likewise generate these forest for our model and compare our results to those reported by Langford et al. (2020).
...
As before, we can also generate a forest plot showing the effect sizes (mean difference) in CFB HbA1c (%) for each treatment in comparison to the Metformin (2000mg) treatment. This can be done using the following code.
...
Within this tutorial we have solely considered on metric (CFB in HbA1c). However, [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863) consider several other primary metrics (weight, total daily insulin dose, and systolic blood pressure). Indeed, each of these treatments is unlikely to affect a single metric (e.g., HbA1c) in isolation, therefore it is imperative that the effect of each treatment on multiple metrics is considered. Additionally, consideration of various safety outcomes for each treatment is also important. For instance, a treatment may effectively reduce a metric of interest but also induce unwanted or potentially dangerous side effects. [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863) additionally analyse the safety outcomes of both hypoglycaemia and diabetic ketoacidosis adverse events. Indeed, in their analysis of these safety outcomes, [Langford et al. (2020)](https://dom-pubs.onlinelibrary.wiley.com/doi/10.1111/dom.13863) highlight safety concerns surronding the use of Sotagliflozin despite its apparent efficacy in reducing HbA1c, a finding corroborated by previous analyses. 
